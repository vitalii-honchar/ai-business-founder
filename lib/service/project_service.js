...
const createProjectService = () => {
    ...
    const generateProjectValidation = async (userId, projectId, request) => {
        ...
        if (userProfile.isMaxValidationsPerProjectReached(projectId)) {
            throw new SubscriptionError(`Max validations reached. Your ${userProfile.subscriptionPlan} plan allows ${userProfile.maxValidationsPerProject} validations.`);
        }

        userProfile.usage.increaseValidationsPerProject(projectId);
    ...
    const createProject = async (userId) => {
        ...
        if (userProfile.isMaxProjectsReached()) {
            throw new SubscriptionError(`Max projects reached. Your ${userProfile.subscriptionPlan} plan allows ${userProfile.maxProjects} projects.`);
        }
    ...
    const deleteProject = async (userId, projectId) => {
        ...
        if (userProfile.usage.maxProjects > 0) {
            userProfile.usage.maxProjects--;
            await userProfileService.updateUserProfile(userProfile);
        }
    };
