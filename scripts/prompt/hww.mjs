...
const hwwwPromptUser = `
For the following user input, provide the response as valid JSON only, with no additional text:

User Input:
`.trim();

const createHwwPrompt = (userInput) => {
  return {
    system: hwwwPromptSystem,
    user: `${hwwwPromptUser}${JSON.stringify(userInput, null, 2)}`
  }
};

export { hwwwPromptSystem, hwwwPromptUser };

export default createHwwPrompt;
