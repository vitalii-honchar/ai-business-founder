...
function TargetRevenueSection({ targetRevenue }) {
    if (!targetRevenue) return null;

    return (
        <div className="bg-green-50 rounded-lg p-4 mb-4">
            <h3 className="font-semibold text-gray-800 mb-2">Target Revenue Validation</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-3">
                        <div>
                            ...
                            <p className="text-xl font-bold text-green-600">
                                ${formatNumber(targetRevenue.monthly_target)}
                            </p>
                        </div>
                        <div>
                            ...
                            <div className="flex items-center">
                                <span className={`text-lg font-semibold ${
                                    targetRevenue.feasibility_score >= 7 ? 'text-green-600' :
                                    targetRevenue.feasibility_score >= 4 ? 'text-yellow-600' :
                                    'text-red-600'
                                }`}>
                                    {targetRevenue.feasibility_score}/10
                                </span>
                            </div>
                        </div>
                        <div>
                            ...
                            <p className="text-base font-medium text-gray-800">
                                {targetRevenue.timeline_months} months
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg p-3">
                    <h4 className="font-medium text-gray-800 mb-2">Acquisition Needs</h4>
                    <div className="space-y-2">
                        <div>
                            <p className="text-sm text-gray-600">Customers Needed</p>
                            <p className="text-base font-medium text-gray-800">
                                {formatNumber(targetRevenue.acquisition_needs.customers_needed)}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">Monthly Rate</p>
                            <p className="text-base font-medium text-gray-800">
                                {formatNumber(targetRevenue.acquisition_needs.monthly_acquisition_rate)}/month
                            </p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">Estimated CAC</p>
                            <p className="text-base font-medium text-gray-800">
                                ${formatNumber(targetRevenue.acquisition_needs.estimated_cac)}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="mt-4 bg-white rounded-lg p-3">
                <h4 className="font-medium text-gray-800 mb-2">Pricing Strategy</h4>
                <div className="flex flex-wrap gap-2 mb-3">
                    <div className="bg-green-50 px-3 py-1 rounded-full">
                        <span className="text-sm text-gray-600">Min: </span>
                        <span className="font-medium text-gray-800">
                            ${formatNumber(targetRevenue.pricing_strategy.recommended_price_range.min)}
                        </span>
                    </div>
                    <div className="bg-green-50 px-3 py-1 rounded-full">
                        <span className="text-sm text-gray-600">Max: </span>
                        <span className="font-medium text-gray-800">
                            ${formatNumber(targetRevenue.pricing_strategy.recommended_price_range.max)}
                        </span>
                    </div>
                </div>
                {targetRevenue.pricing_strategy.pricing_constraints.length > 0 && (
                    <div className="space-y-1">
                        <p className="text-sm text-gray-600">Constraints:</p>
                        {targetRevenue.pricing_strategy.pricing_constraints.map((constraint, index) => (
                            <p key={index} className="text-sm text-gray-700 pl-2">• {constraint}</p>
                        ))}
                    </div>
                )}
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    <h4 className="font-medium text-gray-800">Key Requirements</h4>
                    {targetRevenue.key_requirements.map((req, index) => (
                ...
                <div className="space-y-2">
                    <h4 className="font-medium text-gray-800">Timeline Risks</h4>
                    {targetRevenue.timeline_risks.map((risk, index) => (
                        <div key={index} className="bg-white rounded p-2 text-sm text-gray-700">
                            {risk}
                        </div>
                    ))}
            ...
            {targetRevenue.operational_requirements.length > 0 && (
                <div className="mt-4">
                    <h4 className="font-medium text-gray-800 mb-2">Operational Requirements</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {targetRevenue.operational_requirements.map((req, index) => (
                            <div key={index} className="bg-white rounded p-2 text-sm text-gray-700">
                                {req}
                            </div>
                        ))}
            ...
            {/* Scaling Constraints */}
            {targetRevenue.scaling_constraints.length > 0 && (
                <div className="mt-4">
                    <h4 className="font-medium text-gray-800 mb-2">Scaling Constraints</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {targetRevenue.scaling_constraints.map((constraint, index) => (
                            <div key={index} className="bg-white rounded p-2 text-sm text-gray-700">
                                {constraint}
                            </div>
                        ))}
...
function HwwComponent({ hww }) {
    return (
        <>
            ...
            {/* Target Revenue Validation Section - Moved to bottom */}
            {hww.target_revenue_validation && (
                <section className="bg-white rounded-xl shadow-sm sm:p-4">
                    <h2 className="text-2xl font-bold text-gray-800 mb-4">Revenue Validation</h2>
                    <TargetRevenueSection targetRevenue={hww.target_revenue_validation} />
                </section>
            )}
        </>
    );
